  ...
  <script>
    ...
    */
        ...
        parts.forEach(part => {
          ...
          lines.forEach(line => {
            if (line.startsWith("tipo:")) {
              ...
              q.enunciado = line.substring(10).trim();
      ...
      function renderQuestion(direction) {
        ...
        enunciadoDiv.textContent = q.enunciado;
      ...
      showCorrectBtn.addEventListener("click", function() {
        ...
        if (q.tipo === "singlechoice") {
          Array.from(alternativesDiv.children).forEach((altDiv, idx) => {
            // Para cada alternativa exibida, buscamos o objeto original com base no texto
            var altObj = q.alternatives.find(a => a.text === altDiv.textContent);
            if (altObj && altObj.isCorrect) {
          ...
          if (correctPattern) {
            Array.from(alternativesDiv.children).forEach((altDiv, idx) => {
              // Aqui, usamos a ordem original dos índices do padrão
              var sortedAlts = q.alternatives.slice().sort((a, b) => a.id - b.id);
              if (correctPattern[idx] === "X") {
                altDiv.style.color = "green";
              }
            });
          }
    ...
    //gen_questionario("/h5p/questionario3.txt", "27ª OBA - Nível 2");
  </script>
```
