...
![Carnaval dos planetas como uma paródia ao "alinhamento" dos planetas. [Imagem gerada com Qwen2.5]](carnaval_dos_planetas.png){fig-align="center" .responsive-img .preview-image}
...
2.  **Função `fracao_planetas(T, R)`:**
    ...
    - **Loop Temporal:**
        ...
        - **Cálculo da Condição de Visibilidade Noturna:**
            ...
            - `amin = 0`, `amax = 0`: Inicializa as variáveis `amin` e `amax` para rastrear o ângulo mínimo e máximo formado entre a Terra, Vênus (planeta de referência) e os demais planetas.
            - **Loop para os planetas visíveis (excluindo Terra e Vênus):**
                - `for i in range(2, N)`: Este loop itera sobre os planetas a partir de Mercúrio (índice 2), pulando a Terra e Vênus.
                - `ang = angulo(P[0], P[1], P[i])`: Calcula o ângulo `ang` entre o vetor Terra-Vênus e o vetor Terra-Planeta `i` usando a função `angulo`.
            ...
            - `angS = angulo(P[0], P[1], Ps)`: Calcula o ângulo `angS` entre o vetor Terra-Vênus e o vetor Terra-Sol, usando novamente a função `angulo`.
...
3.  **Função `angulo(A, B, C)`:**
    ...
    - Calcula o produto vetorial 2D (`vet`) e o produto escalar (`esc`) dos vetores `AB` e `AC`.
